...
//    public static final TETile[][] CIRCLE_MAP = generateCircularMap(new TETile[WIDTH][HEIGHT]);
    ...
    private static TETile[][] generateOneLobeNeuriteMap(TETile[][] map) {
        initialize(map);
        addOneLobeNeuriteFloor(map);
        addWalls(map);
        return map;
    }
    ...
    public static void addOneLobeNeuriteFloor(TETile[][] map) {
        int xOffset = 10;
        int yOffset = 10;
        ...
        for (int x = 0; x < WIDTH; x++) {
            for (int y = 0; y < HEIGHT; y++) {
                long dist = Math.round(Math.sqrt(Math.pow(x - r1, 2) + Math.pow(y - r1, 2)));
                if (dist < r1) {
                    map[x][y] = new DTile();
                }
            }
        ...
        for (int x = initialX; x < finalX; x++) {
            for (int y = initialY; y < finalY; y++) {
                long dist = Math.round(Math.sqrt(Math.pow(x - initialX - r2, 2) + Math.pow(y - initialY - r2, 2)));
    ...
    public static void addNeuriteFloor(TETile[][] map) {
        ...
        int channelWidth = 4;
        double dendriteX = Math.round(Math.sqrt(Math.pow(r1, 2) - Math.pow(r1 - channelWidth, 2)));
        ...
        for (int i = 1; i <= 4; i++) {
            // Draws a rectangular channel
            for (int y = center + r1; y > center + r1 - channelWidth; y--) {
                for (int x = center; x < center + r1; x++) {
                    // Add regular tiles for the overlaps with the big circle
                    if (x < center + dendriteX){
                        map[x][y] = new DTile();
                    } else {
                        // Add tracker tiles for the extended region
                        map[x][y] = new DTile(i);
            ...
            for (int x = 0; x < WIDTH; x++) {
                for (int y = 0; y < HEIGHT; y++) {
                    if (map2[x][y] instanceof DTile tile) {
                        map[x][y] = new DTile(tile);
                    } else {
